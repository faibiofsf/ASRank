...
public class ASRank {
	...
	private void iniciar() {
		...
		while (iteracao < numeroIteracoes) {
			...
			/*
			 * if(diversidade <= 30 ){ beta = 0; } else if(diversidade >= 57) {
			 * beta = betatemp; }
			 */
			...
			for (int delt = 0; delt < del.length; delt++) {
				...
				for (int sa = 0; sa < pobSelecaoAleatoria.length; sa++) {
					for (int pr = 0; pr < problema.length; pr++) {
						...
						arqSaidas = new FileWriter(
								"..\\ASRank\\src\\Testes\\Execs\\saida_testes_" + problema[pr] + ".txt");
											...
											for (int k2 = 0; k2 < tamColonia.length; k2++) {
												for (int l = 0; l < iteracoes.length; l++) {
													...
													for (int l2 = 0; l2 < numeroExecucoes; l2++) {
														...
														String entrada = "..\\ASRank\\src\\Testes\\" + problema[pr]
																+ ".tsp";
														String saidaPopulacao = "..\\ASRank\\src\\Testes\\Execs\\Testes_"
																+ l2 + "execucao_" + problema[pr]
																+ "_saidaPopulacao tamColonia-" + tamColonia[k2]
																+ "_iteracoes-" + iteracoes[l] + "_selecao-"
																+ selecao[se] + "_alfa-" + alfa[i] + "_beta-" + beta[j]
																+ "_feromonio-" + q[j2] + "_ro-" + ro[k] + "_SA-"
																+ pobSelecaoAleatoria[sa] + "_del-" + del[delt] +".txt";
														String saidaMelhorGlobal = "..\\ASRank\\src\\Testes\\Execs\\Testes_"
																+ l2 + "execucao_" + problema[pr]
																...
																+ "_feromonio-" + q[j2] + "_ro-" + ro[k] + "_SA-"
																+ pobSelecaoAleatoria[sa] + "_del-" + del[delt] +".txt";
														String saidaDiversidade = "..\\ASRank\\src\\Testes\\Execs\\SaidaDiversidade_"
																+ l2 + "execucao_" + problema[pr]
																+ "_saidaMelhorGlobal tamColonia-" + tamColonia[k2]
																+ "_iteracoes-" + iteracoes[l] + "_selecao-"
																+ selecao[se] + "_alfa-" + alfa[i] + "_beta-" + beta[j]
														...
														String texto = "Teste\t" + l2 + "\texecucao\t" + problema[pr]
																...
																+ iteracoes[l] + "\tselecao\t" + selecao[se]
																+ "\talfa\t" + alfa[i] + "\tbeta\t" + beta[j] + "\tQ\t"
																+ q[j2] + "\tro\t" + ro[k] + "\tSA\t"
